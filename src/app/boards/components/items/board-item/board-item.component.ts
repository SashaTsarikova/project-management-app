...
export class BoardItemComponent implements OnInit, AfterViewInit {
  ...
  deleteBoard() {
    const dialogRef = this.dialogService.open(ConfirmationComponent, {data: `delete board ${this.board.title}`})
    dialogRef.afterClosed().subscribe((result) => {
      if (result && this.board.id) {
        this.boardsService.deleteBoardById(this.board.id).subscribe(() => this.boardsService.updateBoards())
      }
    })
  }
}
