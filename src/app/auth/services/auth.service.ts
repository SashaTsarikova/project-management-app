...
export class AuthService {
  private currentUserSubject: BehaviorSubject<string> =
    new BehaviorSubject<string>(localStorage.getItem('currentUser') || '');
  ...
  login(loginUser: ILogin) {
    ...
    return this.http.post(`${PATH}/signin`, loginUser).pipe(
      tap((token: any) => {
  ...
  signup(signupUser: ISignUp) {
    return this.http.post(`${PATH}/signup`, signupUser).pipe(
      switchMap((userData) => {
        const user: ILogin = {
